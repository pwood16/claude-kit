...
#
# Helpful resources for customization ideas:
# - https://github.com/levz0r/claude-code-statusline (bash with detailed git status)
# - https://github.com/sirmalloc/ccstatusline (Node.js with powerline themes)
# - https://github.com/gabriel-dehan/claude_monitor_statusline (Ruby with usage metrics)
# - https://www.aihero.dev/creating-the-perfect-claude-code-status-line (guide)
...
get_git_info() {
    ...
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        elif [ "$staged_count" -gt 0 ] || [ "$unpushed_count" -gt 0 ] || [ "$untracked_count" -gt 0 ]; then
            color="$YELLOW"  # Yellow if staged, unpushed, or untracked
        else
