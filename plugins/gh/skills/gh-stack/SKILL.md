...
name: gh-stack
description: >
  Use when creating, viewing, editing, submitting, syncing, rebasing, merging, or checking
  out a stack of PRs; when asked to split multi-part work into dependent reviewable
  branches; whenever the user mentions a stack, branch layers, dependent PRs, or gh stack;
  or when a stack is checked out.
metadata:
  author: github
  version: "0.1.0"
  license: MIT
  source: https://github.com/github/gh-stack/tree/main/skills/gh-stack
  vendored-commit: ab00aa4a3f2d
  vendored-date: "2026-08-21"
---
...
A stack couples its PRs: layers merge bottom-up, so a review stall or a revert on a lower layer
blocks everything above it. Reach for a stack when one feature splits into dependent layers that
each deserve their own review — schema, then the API that uses it, then the UI that calls it.
Changes that could merge in any order belong in independent PRs against the trunk, where they
review and merge in parallel. A single reviewable change is one ordinary PR.
...
none do. GitHub evaluates branch protection and required checks per PR when the merge runs, so a
failing requirement on any PR in the set fails the whole set; bypassing merge requirements is
unsupported for stacks.
